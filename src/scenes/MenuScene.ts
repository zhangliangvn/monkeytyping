...
type Item = 'play' | 'arcade' | 'level' | 'character' | 'scene' | 'sound' | 'lang'

export class MenuScene implements Scene {
  private items: Item[] = ['play', 'arcade', 'level', 'character', 'scene', 'sound', 'lang']
  private sel = 0
  private timeMs = 0
  ...
  render(ctx: CanvasRenderingContext2D, w: number, h: number): void {
    ...
    drawCharacterFace(ctx, char, w / 2, h * 0.16 + bob, h * 0.15)
    centeredText(ctx, t('app_title', lang), w / 2, h * 0.3, `800 ${Math.round(h * 0.062)}px 'Segoe UI', sans-serif`, '#ffd166')

    // banana + selected character
    centeredText(ctx, `🍌 ${this.ctx.progress.bananas}`, w / 2, h * 0.37,
      `600 ${Math.round(h * 0.03)}px 'Segoe UI'`, '#ffe08a')
    if (char) {
      centeredText(ctx, `${char.emoji} ${char.name[lang]}`, w / 2, h * 0.41,
        `500 ${Math.round(h * 0.025)}px 'Segoe UI'`, withAlpha('#ffffff', 0.75))
    }
